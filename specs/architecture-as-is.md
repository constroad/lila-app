...
- **Guard de grupo (`getGroupReachabilityError`, 409 `GROUP_NOT_IN_SESSION`):** un destino
  ...
  (`el grupo X no pertenece al número Y`): los 75 anteriores decían todos lo mismo y
  resultaron ser los grupos de CONSTROAD hardcodeados en Portal usados como destino/fallback
  para globofas — el 409 fue la única defensa contra esa fuga entre tenants. Lección completa:
  `specs/OBSERVABILITY-ALERTING.spec.md` §32.
- **Grupo de planta ausente (`dispatch-notifications.service.ts`):** el aviso de progreso y
  el de fin de producción van SIEMPRE al Telegram de alertas y al grupo de planta solo si
  `whatsappConfig.plantGroupId` existe. Cuando no existe, el Telegram lleva el aviso
  `⚠️ Sin grupo de planta configurado (whatsappConfig.plantGroupId)…` (`withPlantGroupNotice`):
  es la única pista de por qué no salió por WhatsApp. Portal ya no manda
  `plantGroupTarget`/`adminGroupTarget` (van vacíos); ambos se resuelven acá desde la config
  de la empresa.
