...
function formatPeruTime(value?: string | Date): string {
  if (!value) return '';
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return '';
...
function normalizeFileCandidate(value: unknown): string {
  return String(value || '')
    .split('?')[0]
    .split('#')[0]
    .toLowerCase()
...
function isImageMedia(media: MediaRecord): boolean {
  const mimeType = String(
    media.mimeTye || media.metadata?.mimeType || ''
  ).toLowerCase();
  const candidate = normalizeFileCandidate(
    media.name || media.url || media.metadata?.fileUrl || media.metadata?.lilaAppUrl
  );

  if (mimeType.includes('image')) {
    return !hasVideoExtension(candidate);
  }
...
function isImagePhoto(photo: unknown): photo is ReportPhoto {
  if (!photo || typeof photo !== 'object') {
    return false;
  }

  const candidatePhoto = photo as ReportPhoto & {
    fileUrl?: string;
    lilaAppUrl?: string;
    metadata?: Record<string, unknown>;
    mime?: string;
  ...
  const mimeType = String(
    candidatePhoto.mimeType ||
      candidatePhoto.mime ||
      candidatePhoto.metadata?.mimeType ||
      ''
  ).toLowerCase();
  const candidate = normalizeFileCandidate(
    candidatePhoto.filename ||
      candidatePhoto.name ||
      candidatePhoto.fileUrl ||
      candidatePhoto.url ||
      candidatePhoto.lilaAppUrl
  );
...
function mapMediaToPhoto(media: MediaRecord): ReportPhoto {
  ...
  return {
    ...
    filename: String(media.name || '').trim(),
    mediaId: String(media._id || '').trim(),
    descripcion: String(metadata.descripcion || media.name || '').trim(),
    fecha: media.date ? new Date(media.date).toISOString() : '',
    hora: String(metadata.hora || formatPeruTime(media.date)).trim(),
    category: String(metadata.category || '').trim(),
    codigoMuestra: String(metadata.codigoMuestra || '').trim(),
    mimeType: String(media.mimeTye || metadata.mimeType || '').trim(),
  };
}

function mapMediaToIppPhoto(
  media: MediaRecord,
  dispatchNoteById: Map<string, string>
): ReportPhoto {
  const basePhoto = mapMediaToPhoto(media);
  if (String(media.type || '') !== 'DISPATCH_PICTURES') {
    return basePhoto;
  }

  const dispatchId = String(media.metadata?.dispatchId || '').trim();
  if (!dispatchId) {
    return basePhoto;
  }

  const dispatchNote = dispatchNoteById.get(dispatchId);
  if (!dispatchNote) {
    return basePhoto;
  }
  ...
  return {
    ...basePhoto,
    descripcion:
      !basePhoto.descripcion || basePhoto.descripcion === basePhoto.filename
        ? dispatchNote
        : basePhoto.descripcion,
  };
}

function mergePhotos(existing: ReportPhoto[], incoming: ReportPhoto[]): ReportPhoto[] {
  const mergedPhotos = [...existing];
  const indexByKey = new Map<string, number>();

  mergedPhotos.forEach((photo, index) => {
    const photoKey = String(photo.mediaId || photo.url || `idx-${index}`);
    indexByKey.set(photoKey, index);
  });

  incoming.forEach((photo) => {
    const photoKey = String(photo.mediaId || photo.url || '').trim();
    if (!photoKey) {
      return;
    }

    const existingIndex = indexByKey.get(photoKey);
    if (existingIndex === undefined) {
      indexByKey.set(photoKey, mergedPhotos.length);
      mergedPhotos.push(photo);
      return;
    }

    const currentPhoto = mergedPhotos[existingIndex];
    const currentDescription = String(currentPhoto.descripcion || '').trim();
    const currentFilename = String(currentPhoto.filename || '').trim();
    const shouldReplaceDescription =
      !currentDescription ||
      (currentFilename && currentDescription === currentFilename) ||
      /^DISPATCH_PICTURES_/i.test(currentDescription);

    mergedPhotos[existingIndex] = {
      ...photo,
      ...currentPhoto,
      descripcion: shouldReplaceDescription
        ? String(photo.descripcion || currentPhoto.descripcion || '').trim()
        : currentPhoto.descripcion,
      category: currentPhoto.category || photo.category,
      codigoMuestra: currentPhoto.codigoMuestra || photo.codigoMuestra,
      hora: currentPhoto.hora || photo.hora,
    };
  });
...
function buildIppPhotoPanels(params: {
  currentSchemaData?: Record<string, unknown>;
  dispatchMedia: MediaRecord[];
  dispatchNoteById: Map<string, string>;
  laboratoryMedia: MediaRecord[];
}) {
  const currentPlantSection =
    typeof params.currentSchemaData?.panelFotograficoPlanta === 'object' &&
    params.currentSchemaData?.panelFotograficoPlanta !== null
      ? (params.currentSchemaData.panelFotograficoPlanta as Record<string, unknown>)
      : {};
  const currentLabSection =
    typeof params.currentSchemaData?.panelFotograficoLaboratorio === 'object' &&
    params.currentSchemaData?.panelFotograficoLaboratorio !== null
      ? (params.currentSchemaData.panelFotograficoLaboratorio as Record<string, unknown>)
      : {};
  const currentPlantPhotos = Array.isArray(currentPlantSection.fotos)
    ? currentPlantSection.fotos.filter(isImagePhoto)
    : [];
  const currentLabPhotos = Array.isArray(currentLabSection.fotos)
    ? currentLabSection.fotos.filter(isImagePhoto)
    : [];
  const nextPlantPhotos = mergePhotos(
    currentPlantPhotos,
    params.dispatchMedia
      .filter(isImageMedia)
      .map((media) => mapMediaToIppPhoto(media, params.dispatchNoteById))
  );
  const nextLabPhotos = mergePhotos(
    currentLabPhotos,
    params.laboratoryMedia.filter(isImageMedia).map(mapMediaToPhoto)
  );
...
async function syncIppDispatchReport(params: {
  ...
  const DispatchModel = await getFlexibleModel('Dispatch', 'dispatches');
  const MediaModel = await getFlexibleModel('Media', 'media');
  const TransportModel = await getFlexibleModel('Transport', 'transports');
  ...
  const linkedServices = await ServiceManagementModel.find({
    companyId: params.companyId,
    orderIds: String(params.order._id),
  })
    .sort({ createdAt: -1 })
    .lean();
  ...
  const transportIds = Array.from(
    new Set(dispatches.map((dispatchItem) => String(dispatchItem.transportId || '')).filter(Boolean))
  );
  const transports = transportIds.length
  ...
  const transportMap = new Map(
    transports.map((transport) => [String(transport._id || ''), transport])
  );
  const company = await CompanyModel.findOne({ companyId: params.companyId }).lean();
  const dispatchRows = dispatches.map((dispatchItem) =>
    mapDispatchToIppRow({
      ...dispatchItem,
      plate: transportMap.get(String(dispatchItem.transportId || ''))?.plate || '',
    })
  );
  const [dispatchMedia, laboratoryMedia] = await Promise.all([
    MediaModel.find({
      companyId: params.companyId,
      resourceId: orderId,
      status: 'ACTIVE',
  ...
  const dispatchNoteById = new Map(
    dispatches
      .map((dispatchItem) => [
        String(dispatchItem._id || '').trim(),
        String(dispatchItem.note || '').trim(),
      ] as const)
  ...
  const report =
    reports.find((reportItem) =>
      ...
      reportItem.orderIds.map((value: unknown) => String(value)).includes(orderId)
    ) ||
    (Array.isArray(service.orderIds) && service.orderIds.length <= 1
      ? reports.find(
          (reportItem) =>
            !Array.isArray(reportItem.orderIds) || reportItem.orderIds.length === 0
        )
      : undefined);

  const nextSchemaData = mergeIppSchemaDataWithDispatchRows({
    currentSchemaData: report?.schemaData,
    incomingRows: dispatchRows,
    orderId,
    plantSettings: company?.plantSettings?.productionReport,
  });
  const nextPhotoPanels = buildIppPhotoPanels({
    currentSchemaData: report?.schemaData,
    dispatchMedia,
    dispatchNoteById,
    laboratoryMedia,
  ...
  const isOrderCompleted = dispatches.length > 0
    && dispatches.every((dispatchItem) => dispatchItem.state === 'despachado');
  const nextStatus = isOrderCompleted ? 'completed' : 'draft';
  const nextSchemaDataWithHeader = {
    ...nextSchemaData,
    ...nextPhotoPanels,
    header: {
      ...(typeof nextSchemaData.header === 'object' && nextSchemaData.header !== null
        ? nextSchemaData.header
        : {}),
      fecha: buildIppHeaderDate(syncDate),
    },
  };
  ...
  if (!report) {
    await ReportModel.create({
      companyId: params.companyId,
      date: syncDate,
      executionDate: syncDate,
      orderIds: [orderId],
      schemaData: nextSchemaDataWithHeader,
      serviceManagementId: String(service._id),
      status: nextStatus,
      type: 'IPP',
    });
  } else {
    const currentOrderIds = Array.isArray(report.orderIds)
      ? report.orderIds.map((value: unknown) => String(value))
      : [];
    const nextOrderIds = currentOrderIds.includes(orderId)
      ? currentOrderIds
      : [...currentOrderIds, orderId];

    await ReportModel.updateOne(
      { _id: report._id, companyId: params.companyId },
      {
        $set: {
          executionDate: syncDate,
