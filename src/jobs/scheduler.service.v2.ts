...
class JobSchedulerV2 {
    ...
    try {
      ...
      if (typeof limit === "number") {
        ...
        if (currentCount >= limit) {
          throw new Error(
            `Cronjob limit reached for company ${data.companyId}`,
          );
        }
      }
      ...
      const cronExpressions = normalizeCronExpressions(
        schedule.cronExpression,
        schedule.cronExpressions,
      );

      if (cronExpressions.length > 0) {
        schedule.cronExpression = cronExpressions[0];
        schedule.cronExpressions = cronExpressions;
        schedule.nextRun = calculateEarliestNextRun(
          cronExpressions,
  ...
  async updateJob(
    ...
    try {
      ...
      if (
        ...
        updates.schedule?.cronExpressions
      ) {
        const cronExpressions = normalizeCronExpressions(
          updates.schedule.cronExpression || job.schedule.cronExpression,
          updates.schedule.cronExpressions || job.schedule.cronExpressions,
        );
        job.schedule.cronExpression = cronExpressions[0];
        job.schedule.cronExpressions = cronExpressions;
        job.schedule.nextRun = calculateEarliestNextRun(
          cronExpressions,
  ...
  private async scheduleJob(
    ...
    try {
      ...
      this.scheduledTasks.set(jobId, tasks);

      if (!options.silent) {
        logger.info("[JobScheduler] Scheduled job", {
          jobId,
          name: job.name,
          companyId: job.companyId,
          expressions,
