...
describe("validators", () => {
  ...
  it("valida cronjob con multiples horarios", () => {
    const result = validateCronJobCreate({
      companyId: "constroad",
      name: "Notificacion doble",
      type: "message",
      schedule: {
        cronExpression: "0 9 * * *",
    ...
    expect(result.success).toBe(true);
    expect(result.data?.schedule.cronExpressions).toHaveLength(2);
  });

  it("rechaza horarios multiples invalidos", () => {
    const result = validateCronJobCreate({
      companyId: "constroad",
      name: "Notificacion invalida",
      type: "message",
      schedule: {
        cronExpression: "0 9 * * *",
        cronExpressions: ["mal-formato"],
      },
      message: {
        chatId: "12345",
        body: "Hola mundo",
  ...
  it("rechaza creacion sin companyId", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.errors?.some((error) => error.field === "companyId")).toBe(
      true,
    );
  });
